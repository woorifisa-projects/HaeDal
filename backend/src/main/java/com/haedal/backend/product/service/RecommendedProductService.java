...
public interface RecommendedProductService extends CrudService<Product, Long> {
    ...
    List<Product> filterProductsByServicePurpose(Long userId);

    // 연령대에 따른 추천상품 전체 조회(상위 랭킹 반영 전)
    List<Product> filterProductsByUserAgeGroup(Long userId);
    ...
    List<Product> filterByServicePurpose(List<Product> recommendedByServicePurpose);
