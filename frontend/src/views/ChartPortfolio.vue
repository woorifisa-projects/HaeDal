<template>
  ...
  <div class="cards-container">
    ...
    <div class="card-wrapper">
      <v-card class="mx-auto">
        <div>
         <Doughnut :data="chartData.data" :options="chartData.options" width = "100" height="200"/>
        </div>
         <div style="width: 100%;text-align: left;">
          <v-card-text>
      ...
      <v-card-text>
        <div>투자 수익 : <b class="text-gray-lighten-1"> +{{ totalinvestgain }}원</b></div>
      </v-card-text>

      <v-card-text>
        <div>투자 수익율 : <b class="text-red-lighten-1"> +{{ (parseFloat(((totalinvestgain) / totalinvestment) * 100)).toFixed(2)
        }}%</b></div>
      </v-card-text>
...
const fetchDataAndRenderChart = () => {
    // ... 데이터 처리 로직 ...
const ArrayLabel = labelData.value;
console.log(ArrayLabel);
const ArrayMoney = moneyData.value

console.log("라벨확인용"+ ArrayLabel);
console.log("라벨확인용"+ ArrayMoney);
const testArray = ["우리1","우리2","우리3"];
const testMoneyArray= ["1","2","3"]

    // 업데이트된 데이터로 차트 업데이트
    chartData.data.labels = testArray;
    console.log("라벨데이터"+ArrayLabel);
    chartData.data.datasets[0].data = testMoneyArray;

    // 차트를 다시 렌더링
    // chartRef.value.renderChart(chartData.data, chartData.options);
     if (chartRef.value) {
      chartRef.value.renderChart(chartData.data, chartData.options);
    }
...
onMounted(() => {
  // 컴포넌트가 마운트된 후에 데이터를 가져와 차트를 렌더링합니다.
  fetchDataAndRenderChart()
})

// 데이터가 설정되었을 때 차트를 그리는 함수
// watch(isDataLoaded, (newValue) => {
//   if (newValue) {
//     // 데이터가 설정되었으므로 차트를 렌더링합니다.
//     chartRef.value.renderChart(chartData.data, chartData.options)
//   }
// })
